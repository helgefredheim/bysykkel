...
const Sorteringsknapp: FunctionComponent<{onClick: onClick, aktiv: boolean}> = props => {
  return (
    <button
      onClick={props.onClick}
      type="button"
      className={classnames('stasjonstabell__sorter', {'stasjonstabell__sorter--aktiv': props.aktiv})}>
        {props.children}
    </button>
  );
}
...
const Stasjonstabell: FunctionComponent<{ stasjoner: Stasjoner }> = props => {
    const [sorteringsfelt, setSorteringsfelt] = useState<Sorteringsfelt>(Sorteringsfelt.STASJON);
    const [asc, setAsc] = useState<boolean>(true);

    const getAriaSort = (sorteringsarg: Sorteringsfelt): 'ascending' | 'descending' | undefined => {
      return sorteringsfelt === sorteringsarg ? (asc ? 'ascending' : 'descending') : undefined;
    };
